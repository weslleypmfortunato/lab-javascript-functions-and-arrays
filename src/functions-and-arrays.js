...
function sum(sumArr) {
  ...
  for (let i = 0; i< sumArr.length; i++) {
    if (typeof sumArr[i] === "number") {
      total += sumArr[i] 
    } else if (typeof sumArr[i] === "string") {
      total += sumArr[i].length
    } else if (typeof sumArr[i] === "boolean" && sumArr[i] === true) {
      total += 1
    } else if (typeof sumArr[i] === "object") {
...
function averageWordLength(wordsArr) {
  ...
  // // if (!wordsArr.length) return null  --> essa seria uma opção diferente para a mesma coisa

  // totalStr = 0
  // for (i = 0; i < wordsArr.length; i++) {
  //   totalStr += wordsArr[i].length
  // }
  // return avg = totalStr / wordsArr.length

  wordsLengths = []
  for (let i = 0; i < wordsArr.length; i++) {
    wordsLengths.push(wordsArr[i].length)
  }
...
function doesWordExist(wordsFind, searchedWord) {
  ...
  // OU ************** meu jeito está abaixo ***************
  // if (wordsFind.includes(searchedWord)) return true
  // return false
}
...
function howManyTimes(wordsCount, searchedWord) {
  ...
  for (i = 0; i < wordsCount.length; i++) {
    if (wordsCount[i] === searchedWord) sumWord++
  } return sumWord
}
...
function greatestProduct(matrix) { //Não entendi nada. Achei a resposta no google
  let greatestProd = 0
  let currentProd
  ...
  for (let i = 0; i < matrix.length; i++){
    for (let j = 0; j < matrix[0].length; j++){
      if (j + 3 < matrix.length){
        currentProd = matrix[i][j] * matrix[i][j+1] * matrix[i][j+2] * matrix[i][j+3]
        if (currentProd > greatestProd){
          greatestProd = currentProd
        }
      }
      if (i + 3 < matrix[0].length){
        currentProd = matrix[i][j] * matrix[i+1][j] * matrix[i+2][j] * matrix[i+3][j]
        if (currentProd > greatestProd){
          greatestProd = currentProd
        }
      }
